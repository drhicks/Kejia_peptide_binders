...
class Diffuser():
    ...
    # for an input of distance, how far do you expect it to be from the origin at t
    #  returns 0-indexed whereas you're probably expecting 1 indexed
    def get_expected_residual(self, distance, sample_size=100):
        distance *= self.crd_scale
        return self.eucl_diffuser.diffuse_translations(torch.ones((sample_size,3,3))*distance)[0].mean(axis=(0,2,3)) / self.crd_scale
