...
class Denoise():
    ...
    def get_raw_delta_cas(self, seq, xyz, t, diffusion_mask ):
        '''
        This could be moved into potential manager if desired - bcov

        Function to take a structure (x) and get the raw delta ca values from the potentials
        ...
        Outputs:
            ...
            Ca_deltas (torch.tensor): [L,3] The amount to move each Ca atom
        '''

        if self.potential_manager == None or self.potential_manager.is_empty(): return torch.zeros(xyz.shape[0], 3)

        seq.requires_grad = False
        xyz.requires_grad = False

        # if not seq.grad is None: seq.grad.zero_()
        # if not xyz.grad is None: xyz.grad.zero_()

        Ca_deltas = self.potential_manager.compute_all_raw_delta_cas(seq.detach(), xyz.detach(), t)

        if not diffusion_mask == None:
            Ca_deltas[diffusion_mask,:] = 0

        # check for NaN's 
        if torch.isnan(Ca_deltas).any():
            print('WARNING: NaN in potential raw deltas, replacing with zero grad.')
            Ca_deltas[:] = 0

        return Ca_deltas.detach().numpy()


    def get_next_pose(self, xt, px0, t, diffusion_mask, seq_t, pseq0, fix_motif=True, align_motif=True, skip_diffuser=False):
        """
        ...
        if ( skip_diffuser ):
            return xt, seq_t, torch.ones((L, 10, 2)), px0
        ...
        ca_deltas += self.get_raw_delta_cas(seq_t.clone(), xt.clone(), t, diffusion_mask )
...
def process_target(pdb_path):
    ...
    target_struct['xyz'] -= ca_center
    print("center before", target_struct['xyz'][:,1,:].mean(axis=0))
    hax.hacks().center_target(target_struct, None)
    print("center after", target_struct['xyz'][:,1,:].mean(axis=0))
    xyz = torch.from_numpy(target_struct['xyz'])
...
def get_idx0_hotspots(mappings, ppi_conf, binderlen):
    ...
    hotspot_idx=None
    if binderlen > 0:
        if ppi_conf.hotspot_res is None:
            assert ppi_conf.no_hotspots, "If you really don't want to provide hotspots while doing PPI, specify this with ppi.no_hotspots=True"
        else:
...
class BlockAdjacency():
    ...
    def __init__(self, conf, num_designs):
        ...
        if isinstance(conf.scaffold_list, (omegaconf.listconfig.ListConfig, list)):
            ...
            with open(conf.scaffold_list,'r') as f:
                for line in f:
                    ...
                    if ( len(split) > 1 ):
                        self.contig_list.append([" ".join(split[1:])])
                    list_from_file.append(split[0])
        ...
        self.dont_touch_ss_adj = conf.dont_touch_ss_adj

        self.scaffold_arc = None
        if ( conf.scaffold_arc ):
            self.scaffold_arc = torch.load(conf.scaffold_arc)
    ...
    def get_ss_adj(self, item):
        ...
        if ( self.scaffold_arc ):
            ss = self.scaffold_arc[tag_part + "_ss"]
            adj = self.scaffold_arc[tag_part + "_adj"]
            print(ss.shape)
            print(adj.shape)
        else:
            ss = torch.load(os.path.join(self.scaffold_dir, f'{tag_part}_ss.pt'))
            adj = torch.load(os.path.join(self.scaffold_dir, f'{tag_part}_adj.pt'))
    ...
    def get_scaffold(self):
        ...
        if self.systematic:
            ...
            if ( self.contig_list ):
                contig = self.contig_list[self.item_n]
            self.increment_scaffold()
        else:
            n_item = random.choice(np.arange(len(self.scaffold_list), dtype=int))
            item = self.scaffold_list[int(n_item)]
            if ( self.contig_list ):
                contig = self.contig_list[n_item]
        print("Scaffold constrained based on file: ", item)
        # load files
        ss, adj = self.get_ss_adj(item)
        if self.dont_touch_ss_adj:
            return ss.shape[0], torch.nn.functional.one_hot(ss.long(), num_classes=4), adj, item, contig
