...
def main(args, pose_str, best_of_n=1, sampling_temps=None, return_all=False, fixed_pos_list_by_chain_d=None, bias_AA_dict=None,
                                            bias_by_res_d=None, tied_positions_list=None, decoding_order=None):
    ...
    import importlib
    import inference.bcov_hacks.hacked_protein_mpnn_utils as hacked_protein_mpnn_utils
    importlib.reload(hacked_protein_mpnn_utils)

    from inference.bcov_hacks.hacked_protein_mpnn_utils import loss_nll, loss_smoothed, gather_edges, gather_nodes, gather_nodes_t, cat_neighbors_nodes, _scores, _S_to_seq, tied_featurize, parse_PDB
    from inference.bcov_hacks.hacked_protein_mpnn_utils import StructureDataset, StructureDatasetPDB, ProteinMPNN
    from inference.bcov_hacks.sc_utils import pack_side_chains, build_sc_model 
    ...
    if True or args.pdb_path:
        pdb_dict_list = parse_PDB(pose_str, "fake_name") #args.pdb_path)
    ...
    if fixed_pos_list_by_chain_d is not None:
        ...
        fixed_positions_dict[pdb_dict_list[0]['name']] = fixed_pos_list_by_chain_d

    if bias_by_res_d is not None:
        bias_by_res_dict = {}
        bias_by_res_dict[pdb_dict_list[0]['name']] = bias_by_res_d

    if tied_positions_list is not None:
        tied_positions_dict = {}
        tied_positions_dict[pdb_dict_list[0]['name']] = tied_positions_list
        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            if args.pack_only:
                ...
                if args.score_only:
                    ...
                    return dict(log_p=concat_log_p, S=S_np, mask=mask[0,].cpu().numpy(), design_mask=mask_out, log_odds=log_odds)
                    ...
                    for f in [1]:
                        ...
                        for temp_c, temp in enumerate(temperatures):
                            ...
                            # print(scorez)
                        best_idx = np.argmin(scorez)

                        if ( return_all ):
                            return seqs, scorez
                        else:
                            return seqs[best_idx],scorez[best_idx]
                                    ...
                                    #     pack_side_chains(args.num_packs, sc_model, S_sample[b_ix][None], X[b_ix][None], mask[b_ix][None], batch_clones[0]['name']+f'_seq_{global_b_ix}_packed', base_folder + 'packed', residue_idx[b_ix][None], chain_encoding_all[b_ix][None])
                    if args.save_score:
