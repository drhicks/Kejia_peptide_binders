...
class PotentialManager:
    ...
    def parse_potential_string(self, potstr):
        ...
        setting_dict = {entry.split(':')[0]:":".join(entry.split(':')[1:]) for entry in potstr.split(',')}

        for key in setting_dict:
            if not key == 'type': setting_dict[key] = ast.literal_eval(setting_dict[key].replace("^", ","))
    ...
    def compute_all_raw_delta_cas(self, seq, xyz, t):
        '''
            This is the money call. Take the current sequence and structure information and get the sum of all of the deltas that are being used
        '''

        delta_list = [potential.raw_delta_ca(seq,xyz,t,self.T) for potential in self.potentials_to_apply]
        delta_stack = torch.stack(delta_list, dim=0)

        return torch.sum(delta_stack, dim=0)
