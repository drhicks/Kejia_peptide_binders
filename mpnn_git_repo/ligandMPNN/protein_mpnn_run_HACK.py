...
def main(args):
    ...
    import random
    import os.path
    from protein_mpnn_utils_HACK import loss_nll, loss_smoothed, gather_edges, gather_nodes, gather_nodes_t, cat_neighbors_nodes, _scores, _S_to_seq, tied_featurize, parse_PDB, make_random_rotation, parse_fasta
    ...
    # Check if the input is a valid file path
        ...
        with open(chain_id_input, 'r') as json_file:
            json_list = list(json_file)
        chain_id_dict = [json.loads(json_str) for json_str in json_list]
    else:
        chain_id_dict = None
        print(40 * '-')
        print('chain_id_input is NOT loaded')
    ...
    #                 kj = True
        ...
        if os.path.exists(args.pdb_path):
            lines = open(args.pdb_path,"rb").readlines()
        else:
            lines = args.pdb_path.splitlines()
            for line in lines:
                if kj:
                    extra_lines_to_pdb += line
                if line[:3] == "TER":
