...
def main(args):
    ...
    with torch.no_grad():
        for ix, protein in enumerate(dataset_valid):
            ...
            #xi_value, xi_idx = torch.sort(xi_log_probs_mean, dim=0, decending=True) 
            #mean = torch.gather(mean, dim=0, index=xi_idx[:sss,])
            #concentration = torch.gather(concentration, dim=0, index=xi_idx[:sss,])
            #mix_logits = torch.gather(mix_logits, dim=0, index=xi_idx[:sss,])
            #predicted_real_values = torch.gather(predicted_real_values, dim=0, index=xi_idx[:sss,])
            ...
            Lmax_p = pred_dist.log_prob(predicted_real_values) #[B,L,4]
            
            tmp_types = torch.tensor(restype_atom37_to_rigid_group, device=device)[S_af2]
            ...
            torsions_mask_sum = (torsion_dict["torsion_angles_mask"][:,:,3:].sum(-1)==0).float()
