...
from drh_utils import (xml,
    get_random_interface_residues_by_ddg,
    get_sap,
...
# Initialize parser
argparser = argparse.ArgumentParser(description="Design proteins using MPNN", formatter_class=argparse.ArgumentDefaultsHelpFormatter)

# Paths and file arguments
argparser.add_argument("--checkpoint_path", default='/databases/mpnn/vanilla_model_weights/v_48_020.pt', help="Path to the model checkpoint.")
argparser.add_argument("--pdb_path", default='', help="Used internally only to pass pdb to mpnn.")
argparser.add_argument("--pdb_path_chains", default='', help="Define which chains need to be designed for a single PDB.")
...
argparser.add_argument("--num_connections", type=int, default=48, help="Number of connections for the model.")

# Sequence generation arguments
argparser.add_argument("--backbone_noise", type=float, default=0.0, help="Standard deviation of Gaussian noise to add to backbone atoms.")
argparser.add_argument("--num_seq_per_target", type=int, default=5, help="Number of sequences to sample per temperature per target.")
argparser.add_argument("--sampling_temp", type=str, default="0.01 0.1 0.2", help="A string of sampling temperatures, 0.2 0.25 0.5")
...
# Amino acid manipulation arguments
argparser.add_argument("--omit_AAs", type=str, default='CX', help="Amino acids to omit in the generated sequence, e.g., 'AC'.")
argparser.add_argument("--bias_AA_jsonl", default='/home/drhicks1/scripts/proteinmpnn/design_scripts/bias_AA.jsonl', help="Path to AA composition bias dictionary.")
...
argparser.add_argument("--contact_patch_filter", action="store_true", help="Enable rosetta relax for binder.")
...
# Replace placeholders in the XML with actual arguments
xml = xml.replace('<Slice name="patchdock_res" indices="%%patchdock_res%%" selector="chainB" />',
                  f'<Slice name="patchdock_res" indices="{args.patchdock_res}" selector="chainB" />')
...
contact_patch_filter = get_filter_and_check_stochastic('contact_patch')
...
def sequence_optimize(pdbfile, pose, args):
    ...
    random_iface = get_random_interface_residues_by_ddg(pose)
    print(f"random_iface: {random_iface}")
    
    args.fix_a = ",".join(map(str, random_iface))
...
def dl_design( pose, tag, silent_out, sfd_out , args):
    ...
    seqs = sequence_optimize( pdbfile, pose, args )
    ...
    for idx,seq in enumerate( seqs ):
        ...
        add2scorefile( tag, f"{args.out_name}.sc", {'ddg':ddg, 'contact_patch':contact_patch, 'contact_molecular_surface':contact_molecular_surface, 'sap':sap_score} )
